...
public class ClientNaiveAgent implements Runnable {
	...
	public GameState solve() {
		...
		if (sling != null && !pigs.isEmpty()) {
			...
			Point _targetPoint = pig.getCenter();

			// if the target is very close to before, randomly choose a point near it
			if (prevTarget != null && distance(prevTarget, _targetPoint) < 10) {
				double _angle = randomGenerator.nextDouble() * Math.PI * 2;
				_targetPoint.x = _targetPoint.x + (int) (Math.cos(_angle) * 10);
				_targetPoint.y = _targetPoint.y + (int) (Math.sin(_angle) * 10);
				System.out.println("Randomly changing to " + _targetPoint);
			}

			prevTarget = new Point(_targetPoint.x, _targetPoint.y);

			// estimate the trajectory
			ArrayList<Point> pts = tp.estimateLaunchPoint(sling, _targetPoint);
			...
			if (firstShot && pts.size() > 1) {
				...
				releasePoint = pts.get(randomGenerator.nextInt(6) == 0 ? 1 : 0);
			}
			...
			if (releasePoint != null) {
				...
				tapTime = tp.getTapTime(sling, releasePoint, _targetPoint, tapInterval);
			} else {
